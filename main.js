...
window.addEventListener( "load", () => {
  ...
  if (verificar()) {
    insetarTodosLosComentarios(comentario, commentBox);
  } else {    
...
function insetarTodosLosComentarios(data, element) {
  commentNumber.innerText = Number( commentNumber.innerText ) + data.length;
  for (const comentario of data) {
    let miDiv = document.createElement("div");
    miDiv.className = "facebook-box";
    miDiv.innerHTML = templateComentario(comentario, 1);
...
function verificar() {
  ...
  let status = localStorage.getItem('views')
